...
#' @param by_year boolean. Summarise by year
#' @export
#' @rdname extreme_events
drought = function(data, data_GS, threshold_growth = 4, threshold_prec = 0, by_year = TRUE)
{
  KeyID <- Year <- yday <- GSstart <- GSend <- GS_cond <- Tair <- growing_dd <- Prcp <- drought_dd <- droughtevent_dd <- NULL
  ...
  data <- data |> dplyr::mutate(drought_dd = cumsum(ifelse(GS_cond & Tair > threshold_growth & Prcp <= threshold_prec, growing_dd, 0)), #somme des degrés-jours pour toutes les sécheresses de la saison
           droughtevent_dd = cumsum_reset(ifelse(GS_cond & Tair > threshold_growth & Prcp <= threshold_prec, growing_dd, 0), reset =  0)) #somme des degrés-jour par événement de sécheresse
